...
export function Home({ progress, onSelectLevelMode, onSelectRandomMode, onBadgesClick }) {
  ...
  return (
    <div className="mx-auto max-w-2xl h-screen px-5 py-6 sm:py-8 flex flex-col overflow-hidden">
      <div className="space-y-5 sm:space-y-8">
        ...
        >
          <p className="font-label text-marigold text-xs sm:text-sm tracking-[0.3em] uppercase mb-1.5">{t('app.tagline')}</p>
          <h1 className="font-display text-4xl sm:text-6xl text-maroon-deep leading-[1.05]">
            भगवान महावीर के<br /> पथ पर चलें
          </h1>
          <p className="mt-3 sm:mt-4 text-sm sm:text-base text-indigo/70 max-w-md mx-auto">
            {t('home.description')}
          </p>
        </motion.div>
        ...
        <div className="grid gap-3 sm:gap-4 sm:grid-cols-2">
          <motion.div whileHover={{ y: -4 }} whileTap={{ scale: 0.98 }}>
            ...
            >
              ...
              <CardContent className="relative space-y-2 sm:space-y-3">
                <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-lg sm:rounded-xl bg-maroon flex items-center justify-center text-parchment">
                  <Map size={20} className="sm:w-6 sm:h-6" />
                </div>
                <div>
                  <h3 className="font-display text-xl sm:text-2xl text-maroon-deep">{t('home.storyMode')}</h3>
                  <p className="text-xs sm:text-sm text-indigo/60 mt-1 leading-relaxed">
                ...
                <div className="flex items-center justify-between pt-1">
                  ...
                  <ChevronRight size={16} className="sm:w-5 sm:h-5 text-maroon" />
                </div>
              </CardContent>
            </Card>
          </motion.div>
          ...
          <motion.div whileHover={{ y: -4 }} whileTap={{ scale: 0.98 }}>
            ...
            >
              ...
              <CardContent className="relative space-y-2 sm:space-y-3">
                <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-lg sm:rounded-xl bg-marigold flex items-center justify-center text-indigo">
                  <Shuffle size={20} className="sm:w-6 sm:h-6" />
                </div>
                <div>
                  <h3 className="font-display text-xl sm:text-2xl text-maroon-deep">{t('home.endlessWander')}</h3>
                  <p className="text-xs sm:text-sm text-indigo/60 mt-1 leading-relaxed">
        ...
        <Card className="bg-indigo/5 border-indigo/10">
          <CardContent className="flex items-center gap-3 sm:gap-4">
            <Trophy size={22} className="sm:w-7 sm:h-7 text-marigold shrink-0" />
            <p className="text-xs sm:text-sm text-indigo/70">
              {t('home.scoringInfo')}
            </p>
          </CardContent>
        </Card>

        <p className="text-center text-xs text-indigo/40 pb-2">{t('home.progressSaved')}</p>
      </div>
    </div>
  )
}
